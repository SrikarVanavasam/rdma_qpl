...
qpl_status hw_check_compress_job(qpl_job* qpl_job_ptr) {
    ...
    std::cout << "  decomp_flags: 0x" << std::hex << desc_ptr->decomp_flags << std::dec << std::endl;
    std::cout << "  ADCF_STATS_MODE: 0x" << std::hex << ADCF_STATS_MODE << std::dec << std::endl;
    ...
    if (qpl_job_ptr->numa_id == qpl::rdma::QPL_RDMA_REMOTE_NUMA_ID) {
        ...
        if (!rdma_client_instance || !rdma_client_instance->is_initialized()) {
            if (const char* env_ip = std::getenv("QPL_RDMA_SERVER_IP")) {
                ...
                std::cerr << "[QPL] QPL_RDMA_REMOTE_NUMA_ID used but QPL_RDMA_SERVER_IP not set in check_job."
                          << std::endl;
                return QPL_STS_INIT_HW_NOT_SUPPORTED;
            }
        }
        ...
        if (output_size > 0 && qpl_job_ptr->next_out_ptr != nullptr) {
            ...
            if (output_size > qpl_job_ptr->available_out) {
                std::cerr << "[QPL] Warning: Output overflow! Size " << output_size << " > Available "
                          << qpl_job_ptr->available_out << ". Truncating read." << std::endl;
