...
int cxl_iaa_expand(std::string src_data_file_path, const CxlBenchConfig& config, const uint32_t queue_size)
{
    std::cout << "[CXL IAA Expand]" << std::endl;
    ...
    size_t mask_byte_length = chunk_size / 8;

    // Use 1GB Hugepages for destination and mask
    ...
    uint8_t* all_mask_buffer = (uint8_t*)huge_alloc_on_node(queue_size * mask_byte_length, config.numa_node, true);
    uint64_t all_mask_iova;
    qpl_cxl_register_buffer(all_mask_buffer, queue_size * mask_byte_length, &all_mask_iova);

    for (int i = 0; i < queue_size; ++i) {
        dest_ptrs[i] = all_dest_buffer + i * chunk_size;
        mask_ptrs[i] = all_mask_buffer + i * mask_byte_length;
        std::memset(mask_ptrs[i], 0xFF, mask_byte_length); // Simple all-ones mask
    }
    ...
    while(src_file_left > 0) {
        int enqueue_cnt = 0;
        for (int i = 0; i < queue_size; ++i) {
            size_t vector_size = (src_file_left <= chunk_size / 2) ? src_file_left : chunk_size / 2;
            ...
            job[i]->available_src2     = static_cast<uint32_t>(mask_byte_length);
            job[i]->next_src2_ptr      = mask_ptrs[i];
            job[i]->num_input_elements = static_cast<uint32_t>(chunk_size);
    ...
    std::cout << "\nExpand performed successfully." << std::endl;
    ...
    huge_free(all_mask_buffer, queue_size * mask_byte_length, true);
...
int main(int argc, char** argv) {
    ...
    return cxl_iaa_expand(src_file, config, queue_size);
}
