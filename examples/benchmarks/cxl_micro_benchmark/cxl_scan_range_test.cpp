...
int cxl_iaa_scan_range(std::string src_data_file_path, const CxlBenchConfig& config, const uint32_t queue_size)
{
    std::cout << "[CXL IAA Scan Range]" << std::endl;
    ...
    // Use 1GB Hugepages for destination
    ...
    std::cout << "\nScan Range performed successfully." << std::endl;
...
int main(int argc, char** argv) {
    ...
    return cxl_iaa_scan_range(src_file, config, queue_size);
}
