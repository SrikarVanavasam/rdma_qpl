//* [QPL_CXL_DECOMPRESSION_EXTRACT_BENCHMARK] */
...
static int32_t lower_index = 0;
static int32_t upper_index = 2097151;

int cxl_iaa_decompression_extract(std::string compressed_data_prefix, uint32_t num_files, const CxlBenchConfig& config, const uint32_t queue_size)
{
    std::cout << "[CXL IAA Decompression + Extract]" << std::endl;
    ...
    std::cout << "\nDecompression + Extract performed successfully." << std::endl;
...
int main(int argc, char** argv) {
    ...
    if (argc < next_arg + 4) {
        std::cerr << "Usage: " << argv[0] << " <mode> <numa_node> [server_ip] <prefix> <num_files> <lower_idx> <upper_idx> [queue_size]\n";
        return 1;
    }

    std::string prefix = argv[next_arg];
    uint32_t num_files = std::stoi(argv[next_arg + 1]);
    lower_index = std::stoi(argv[next_arg + 2]);
    upper_index = std::stoi(argv[next_arg + 3]);
    uint32_t queue_size = 64;

    if (argc > next_arg + 4) {
        queue_size = std::stoi(argv[next_arg + 4]);
    }
    ...
    return cxl_iaa_decompression_extract(prefix, num_files, config, queue_size);
}
