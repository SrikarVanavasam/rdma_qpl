...
int cxl_iaa_extract(std::string src_data_file_path, const CxlBenchConfig& config, const uint32_t queue_size)
{
    std::cout << "[CXL IAA Extract]" << std::endl;
    ...
    // Use 1GB Hugepages for all destination chunks
    ...
    std::cout << "\nExtract performed successfully." << std::endl;
...
int main(int argc, char** argv) {
    ...
    if (argc < next_arg + 3) {
        std::cerr << "Usage: " << argv[0] << " <mode> <numa_node> [server_ip] <src_file> <lower_idx> <upper_idx> [queue_size] [chunk_size]\n";
        return 1;
    }

    std::string src_file = argv[next_arg];
    lower_index = std::stoi(argv[next_arg + 1]);
    upper_index = std::stoi(argv[next_arg + 2]);
    
    uint32_t queue_size = 64;
    chunk_size = 2097152; // 2MB

    if (argc > next_arg + 3) {
        queue_size = std::stoi(argv[next_arg + 3]);
    }
    if (argc > next_arg + 4) {
        chunk_size = std::stoul(argv[next_arg + 4]);
    }
    ...
    return cxl_iaa_extract(src_file, config, queue_size);
}
